...
class AttackingBall(Ball):
    ...
    def accelerate_to_target(self, dt):
        relative_velocity = (
            self.velocity['x'] - self.target.velocity['x'],
            self.velocity['y'] - self.target.velocity['y']
        )
        distance = (
        ...
        if hypot(*acceleration_direction) < 0.2:
            acceleration_direction = distance_direction
